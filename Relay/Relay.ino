...
int filterPin(int input)
{
  int pin[] = {14, 27, 26, 25, 33, 32};
  int i;
  for(i = 0; i<6; i++)
  {
    if(pin[i] == input)
...
void restorePinState()
{
  ...
  for(i = 0; i<6; i++)
  {
    String str = readPinState(pin[i]);
    if(str == "0" || str == "1")
    {
      pinMode(pin[i], OUTPUT);
      digitalWrite(pin[i], str.toInt());
    }
  }
}
...
String readPinState(int pin)
{
  int offset = 700 + (pin * 2);
  String val = readDataString(offset, 2);
  return val;
}
